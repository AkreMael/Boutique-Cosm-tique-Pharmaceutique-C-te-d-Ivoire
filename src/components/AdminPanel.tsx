  ...
  return (
    <div id="admin-backoffice-section" className="py-8 bg-zinc-50 min-h-[calc(100vh-80px)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between pb-6 border-b border-rose-100 gap-4 mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-xl text-xs font-bold transition flex items-center space-x-1.5 cursor-pointer ${
                adminTab === 'modules' ? 'bg-zinc-900 text-white shadow-sm' : 'bg-white hover:bg-zinc-50 text-zinc-700'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-xl text-xs font-bold transition flex items-center space-x-1.5 cursor-pointer ${
                adminTab === 'items' ? 'bg-zinc-900 text-white shadow-sm' : 'bg-white hover:bg-zinc-50 text-zinc-700'
              }`}
            >
              <List className="h-4 w-4 text-emerald-500" />
              <span>Annonces ({items.length})</span>
            </button>
            <button
            ...
            >
              ...
              <span>Rôles & Accès ({allUsers.length})</span>
            </button>
            <button
              onClick={() => setAdminTab('logs')}
              className={`px-4 py-2.5 rounded-xl text-xs font-bold transition flex items-center space-x-1.5 cursor-pointer ${
                adminTab === 'logs' ? 'bg-zinc-900 text-white shadow-sm' : 'bg-white hover:bg-zinc-50 text-zinc-700'
              }`}
            >
        ...
        {/* ==================================== */}
        {/* TAB 8: CONSOLE DES MODULES ACTIFS   */}
        {/* ==================================== */}
        {adminTab === 'modules' && (
          <div className="bg-white rounded-3xl p-6 border border-zinc-100 shadow-sm animate-fade-in space-y-6">
            <div>
              <h3 className="font-extrabold text-rose-950 text-sm mb-1">Régulation des Modules Platforme (Abonnement Général)</h3>
              <p className="text-zinc-400 text-xs">Contrôlez l'ouverture, la visibilité et l'activation des branches applicatives pour tous les clients du portail.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {modules.length === 0 ? (
                <div className="col-span-3 text-center py-10 bg-zinc-50 rounded-2xl text-xs text-zinc-400">
                  Aucun module applicatif n'est actuellement repéré dans la base d'évaluation.
                </div>
              ) : (
                modules.map(mod => (
                  <div key={mod.id} className="p-5 bg-zinc-50 rounded-2xl border border-zinc-150 flex flex-col justify-between gap-4">
                    <div>
                      <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-rose-650 bg-rose-50 px-2 py-0.5 rounded-md inline-block mb-2">
                        Type : {mod.type}
                      </span>
                      <h4 className="text-xs font-black text-rose-950">{mod.name}</h4>
                      <p className="text-[11px] text-zinc-500 mt-1">Status opérationnel : <span className={`font-bold ${mod.status === 'ACTIVE' ? 'text-emerald-600' : 'text-zinc-455'}`}>{mod.status}</span></p>
                    </div>

                    <button
                      onClick={() => handleToggleModuleStatus(mod.id, mod.status)}
                      className={`w-full py-2 rounded-xl text-xs font-bold transition flex items-center justify-center space-x-1.5 cursor-pointer ${
                        mod.status === 'ACTIVE'
                          ? 'bg-rose-50 hover:bg-rose-100 text-rose-950 border border-rose-150'
        ...
        {/* ==================================== */}
        {/* TAB 9: GESTION DES ANNONCES PUBLIÉES */}
        {/* ==================================== */}
        {adminTab === 'items' && (
          <div className="bg-white rounded-3xl p-6 border border-zinc-100 shadow-sm animate-fade-in space-y-6">
            <div>
              <h3 className="font-extrabold text-rose-950 text-sm mb-1">Destitution & Audit d'Annonces Immobilières et de Services</h3>
              <p className="text-zinc-400 text-xs">Examinez ou destituez les annonces non conformes publiées en temps réel par les usagers sur la boutique Akwaba.</p>
            </div>
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-zinc-50 border-b border-zinc-150">
                    <th className="p-3.5 font-bold text-zinc-600">Titre de l'Annonce</th>
                    <th className="p-3.5 font-bold text-zinc-600">Module</th>
                    <th className="p-3.5 font-bold text-zinc-600">Propriétaire</th>
                    <th className="p-3.5 font-bold text-zinc-600">Tarif Indiqué</th>
                    <th className="p-3.5 font-bold text-zinc-600 text-right">Actions administratives</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-150">
                  {items.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="p-8 text-center text-zinc-400 font-medium">Aucune annonce (Immobilier, Service) recensée dans la base de données.</td>
                    </tr>
                  ) : (
                    items.map(item => (
                      <tr key={item.id} className="hover:bg-zinc-50/50">
                        <td className="p-3.5">
                          <div className="flex items-center gap-3">
                            <span className="h-8 w-8 rounded-lg overflow-hidden shrink-0 bg-zinc-100">
                              <img src={item.images?.[0]} className="h-full w-full object-cover" alt="" />
                            </span>
                            <div>
                              <span className="font-bold text-zinc-800 block leading-tight">{item.title}</span>
                              <span className="text-[10px] text-zinc-400 font-mono mt-0.5 block">{item.id}</span>
                            </div>
                          </div>
                        </td>
                        <td className="p-3.5">
                          <span className={`px-2 py-0.5 rounded-md text-[10px] font-mono font-bold uppercase ${item.moduleId === 'rental' ? 'bg-indigo-50 text-indigo-700 border border-indigo-150' : 'bg-emerald-50 text-emerald-700 border border-emerald-150'}`}>
                            {item.moduleId === 'rental' ? 'Immobilier' : 'Prestation'}
                          </span>
                        </td>
                        <td className="p-3.5">
                          <div>
                            <span className="font-bold text-zinc-700 block">{item.ownerName}</span>
                            <span className="text-[10px] text-zinc-400 font-mono mt-0.5 block">{item.ownerPhone}</span>
                          </div>
                        </td>
                        <td className="p-3.5">
                          <span className="font-mono font-extrabold text-rose-950">{item.price.toLocaleString()} XOF</span>
                        </td>
                        <td className="p-3.5 text-right">
                          <button
                            onClick={() => handleDeleteListingItem(item.id, item.title)}
                            className="bg-red-50 hover:bg-red-100 text-red-700 p-2 rounded-xl transition cursor-pointer"
                            title="Destituer l'annonce"
                          >
                            <Trash2 className="h-4 w-4" />
                          </button>
                        </td>
        ...
        {/* ==================================== */}
        {/* TAB 10: RÔLES & ABONNEMENTS CLIENTS */}
        {/* ==================================== */}
        {adminTab === 'users' && (
          <div className="bg-white rounded-3xl p-6 border border-zinc-100 shadow-sm animate-fade-in space-y-8">
            
            {/* User Roles Management Panel */}
            <div className="space-y-4">
              <div>
                <h3 className="font-extrabold text-rose-950 text-sm mb-1">Annuaire d'Usagers & Attribution de Rôles</h3>
                <p className="text-zinc-400 text-xs">Examinez les profils d'usagers de Côte d'Ivoire, visualisez l'audit de vérification de numéros, et commutez les rôles d'accès.</p>
              </div>
              ...
              <div className="relative max-w-md">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-zinc-50 border border-zinc-200 rounded-xl text-xs text-zinc-700 focus:outline-none"
                />
              </div>
              ...
              <div className="overflow-x-auto rounded-2xl border border-zinc-150">
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="bg-zinc-50 border-b border-zinc-150">
                      <th className="p-3.5 font-bold text-zinc-600">Nom complet / Identité</th>
                      <th className="p-3.5 font-bold text-zinc-600">Numéro CI / Vérifié</th>
                      <th className="p-3.5 font-bold text-zinc-600">Ville</th>
                      <th className="p-3.5 font-bold text-zinc-600">Courriel (Email)</th>
                      <th className="p-3.5 font-bold text-zinc-600">Rôle Actuel</th>
                      <th className="p-3.5 font-bold text-zinc-600 text-right">Modifier Rôle</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-150">
                    {allUsers
                        ...
                        <tr key={usr.id} className="hover:bg-zinc-50/50">
                          <td className="p-3.5">
                            <div className="flex items-center gap-2.5">
                              <span className="h-8 w-8 rounded-full bg-zinc-100 overflow-hidden border border-zinc-200 shrink-0 flex items-center justify-center font-bold text-zinc-700">
                                {usr.name ? usr.name.substring(0, 1).toUpperCase() : 'U'}
                              </span>
                              <div>
                                <span className="font-bold text-zinc-800 block">{usr.name}</span>
                                <span className="text-[10px] text-zinc-400 font-mono mt-0.5 block">{usr.id}</span>
                              </div>
                            </div>
                          </td>
                          <td className="p-3.5">
                            <div className="space-y-1">
                              <span className="font-mono text-zinc-700 font-bold block">{usr.phone}</span>
                              {usr.isPhoneVerified ? (
                                <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 text-[9px] font-bold px-1.5 py-0.5 rounded-full border border-emerald-150">
                                  Vérifié par OTP
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 text-[9px] font-bold px-1.5 py-0.5 rounded-full border border-amber-150">
                                  Non vérifié (OTP)
                                </span>
                              )}
                            </div>
                          </td>
                          <td className="p-3.5 text-zinc-650">{usr.city}</td>
                          <td className="p-3.5 text-zinc-600 font-mono select-all">{usr.email}</td>
                          ...
                          <td className="p-3.5 text-right">
                            <button
                              onClick={() => handleUpdateUserRole(usr.id, usr.name, usr.role)}
                              className="px-3 py-1.5 bg-zinc-100 hover:bg-zinc-200 rounded-lg text-[10px] font-black uppercase tracking-wider transition cursor-pointer"
                            >
                              Commuter rôle
                            </button>
                          </td>
            ...
            {/* Access subscriptions Management panel */}
            <div className="space-y-4 pt-6 border-t border-zinc-150">
              <div>
                <h3 className="font-extrabold text-rose-950 text-sm mb-1">Validation des Accès spécifiques Modules (UserModuleAccess)</h3>
                <p className="text-zinc-400 text-xs">Validez ou refusez les demandes de droit d'accès et d'abonnements formulées par les membres de la plateforme.</p>
              </div>
              ...
              <div className="overflow-x-auto rounded-2xl border border-zinc-150">
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="bg-zinc-50 border-b border-zinc-150">
                      <th className="p-3.5 font-bold text-zinc-600">ID d'Abonné</th>
                      <th className="p-3.5 font-bold text-zinc-600">Identifiant Module</th>
                      <th className="p-3.5 font-bold text-zinc-600">Niveau d'Autorisation</th>
                      <th className="p-3.5 font-bold text-zinc-600 text-right">Remplacer statut d'accès</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-150">
                    {moduleAccess.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="p-8 text-center text-zinc-400 font-semibold">Aucune réclamation d'autorisation ou d'abonnement en attente.</td>
                      </tr>
                    ) : (
                      moduleAccess.map(acc => (
                        <tr key={acc.id} className="hover:bg-zinc-50/50">
                          <td className="p-3.5 font-mono text-zinc-650">{acc.userId}</td>
                          <td className="p-3.5 font-mono text-zinc-650">{acc.moduleId}</td>
                          <td className="p-3.5">
                            <span className={`px-2.5 py-1 rounded-xl text-[10px] font-bold ${
                              acc.accessLevel === 'GRANTED'
                                ? 'bg-emerald-50 text-emerald-700 border border-emerald-150'
                                : acc.accessLevel === 'PENDING_SUBSCRIBED'
                          ...
                          <td className="p-3.5 text-right">
                            <button
                              onClick={() => handleToggleAccessLevel(acc.id, acc.accessLevel)}
                              className="px-3.5 py-1.5 bg-rose-50 text-rose-950 border border-rose-150 rounded-lg text-[10px] font-extrabold uppercase tracking-wide cursor-pointer hover:bg-rose-100 transition"
                            >
                              Commuter Droits
                            </button>
                          </td>
        ...
        {/* ==================================== */}
        {/* TAB 11: HISTORIQUE AUDIT LOGS PAGE */}
        {/* ==================================== */}
        {adminTab === 'logs' && (
          <div className="bg-white rounded-3xl p-6 border border-zinc-100 shadow-sm animate-fade-in space-y-6">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div>
                <h3 className="font-extrabold text-rose-950 text-sm mb-1">Historique d'Activités & Audit Clinique</h3>
                <p className="text-zinc-400 text-xs font-normal">Journal complet d'interactions techniques, modifications de profils, publications et transactions formulées sur Akwaba.</p>
              </div>
              ...
              <div className="relative w-full md:w-72">
                ...
                <input
                  type="text"
                  placeholder="Filtrer par action ou usager..."
            ...
            <div className="space-y-4 max-h-[500px] overflow-y-auto pr-1">
              {activityLogs
                ...
                .map(log => (
                  <div key={log.id} className="p-4 bg-zinc-50 border border-zinc-150 hover:border-zinc-200 transition rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-3 text-xs">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="h-2 w-2 rounded-full bg-rose-500 shadow-sm shrink-0"></span>
                        <span className="font-extrabold text-rose-950">{log.action}</span>
                        <span className="text-[10px] text-zinc-400">• Par {log.userName} (ID: {log.userId})</span>
                      </div>
                      <p className="text-zinc-550 mt-1.5 leading-relaxed pl-4 font-normal">{log.details}</p>
                    </div>

                    <span className="text-[10px] text-zinc-400 font-mono shrink-0 pl-4 md:pl-0 align-right self-end md:self-center">
