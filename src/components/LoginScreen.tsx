...
export default function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  const handleClientSubmit = (e: React.FormEvent) => {
    ...
    if (!clientName.trim() || !clientUsername.trim() || !clientCity.trim() || !clientPhone.trim()) {
      setError("Veuillez remplir tous les champs.");
      return;
    }

    // Exact regex: +225 followed by exactly 10 digits (which in CI start with 01, 05, 07, etc)
    // Strip empty spaces to validate digits
    ...
    const appUser: AppUser = {
      ...
      email: `${clientUsername.trim()}@cosmetiques.ci`,
  ...
  return (
    <div id="auth-portal" className="min-h-screen bg-zinc-50 flex items-center justify-center p-4 selection:bg-rose-100 font-sans">
      ...
      <motion.div 
        ...
        className="w-full max-w-md bg-white border border-rose-100/70 rounded-[2.5rem] p-8 md:p-10 shadow-2xl relative z-10 space-y-8"
      >
        {/* Header Branding */}
        ...
        {activeTab === 'client' ? (
          <form onSubmit={handleClientSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="block text-zinc-650 text-xs font-bold font-sans">Nom d'utilisateur *</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-zinc-400">
